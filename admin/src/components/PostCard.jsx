...
export default function PostCard({ post, onDeleteClick }) {
  ...
  return (
    <div className="bg-white shadow-sm rounded p-4 flex flex-col">
      <img
      ...
      <div className="p-2 flex-1 flex-col justify-between">
        ...
        <div className="flex space-x-3">
          <Link
            to={`/update-post/${slug}`}
            className="w-8 h-8 rounded-full bg-blue-400 hover:bg-blue-600 flex justify-center items-center"
          >
            <BsPencilSquare />
          </Link>
          <button
            onClick={onDeleteClick}
            className="w-8 h-8 rounded-full bg-red-400 hover:bg-red-600 flex justify-center items-center"
          >
            <BsTrash />
          </button>
        </div>
